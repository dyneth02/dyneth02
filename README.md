<table border="0" cellpadding="0" cellspacing="0" align="center" style="border-collapse: collapse; border: none;">
    <tr>
        <td align="right" valign="center" style="padding: 10px; border: none;">
            <div style="color:#b78dff; font-family:'Courier New', monospace; white-space:pre; line-height:1.2; font-weight:bold;">
██╗  ██╗███████╗██╗     ██╗      ██████╗   <br>
██║  ██║██╔════╝██║     ██║     ██╔═══██╗  <br>
███████║█████╗  ██║     ██║     ██║   ██║  <br>
██╔══██║██╔══╝  ██║     ██║     ██║   ██║  <br>
██║  ██║███████╗███████╗███████╗╚██████╔╝  <br>
╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝ ╚═════╝   <br>
            </div>
            <div style="color:#b78dff; font-family:'Courier New', monospace; white-space:pre; line-height:1.2; font-weight:bold;">
███████╗███████╗███████╗██╗  ██╗███████╗██████╗   <br>
██╔════╝██╔════╝██╔════╝██║ ██╔╝██╔════╝██╔══██╗  <br>
███████╗█████╗  █████╗  █████╔╝ █████╗  ██████╔╝  <br>
╚════██║██╔══╝  ██╔══╝  ██╔═██╗ ██╔══╝  ██╔══██╗  <br>
███████║███████╗███████╗██║  ██╗███████╗██║  ██║  <br>
╚══════╝╚══════╝╚══════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝  <br>
            </div>
        </td>
        <td align="center" valign="center" style="padding: 10px; border: none;">
            <img src="https://raw.githubusercontent.com/dyneth02/dyneth02/main/resources/giphy.gif" width="220" style="border-radius: 50%;">
        </td>
    </tr>
    <tr>
        <td align="center" valign="center" style="padding: 10px; border: none;">
            <img src="https://raw.githubusercontent.com/dyneth02/dyneth02/main/resources/space-music.gif" width="220" style="border-radius: 50%;">
        </td>
        <td align="left" valign="center" style="padding: 10px; border: none;">
            <div style="color:#b78dff; font-family:'Courier New', monospace; white-space:pre; line-height:1.2; font-weight:bold;">
██████╗ ██╗███╗   ██╗███████╗████████╗██╗  ██╗  <br>
██╔══██╗██║████╗  ██║██╔════╝╚══██╔══╝██║  ██║  <br>
██║  ██║██║██╔██╗ ██║█████╗     ██║   ███████║  <br>
██║  ██║██║██║╚██╗██║██╔══╝     ██║   ██╔══██║  <br>
██████╔╝██║██║ ╚████║███████╗   ██║   ██║  ██║  <br>
╚═════╝ ╚═╝╚═╝  ╚═══╝╚══════╝   ╚═╝   ╚═╝  ╚═╝  <br>
            </div>
            <div style="color:#b78dff; font-family:'Courier New', monospace; white-space:pre; line-height:1.2; font-weight:bold;">
██╗  ██╗███████╗██████╗ ███████╗  <br>
██║  ██║██╔════╝██╔══██╗██╔════╝  <br>
███████║█████╗  ██████╔╝█████╗    <br>
██╔══██║██╔══╝  ██╔══██╗██╔══╝    <br>
██║  ██║███████╗██║  ██║███████╗  <br>
╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚══════╝  <br>
            </div>
        </td>
    </tr>
</table>

<p align="center">
    <img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">
</p>

<div align="center">
    <h2 style="font-family: sans-serif;"> About Myself</h2>
    <p style="width:400px; text-align: justify; font-family: sans-serif;" width="100">
        I design AI, ML, and data systems where uncertainty is treated as a constraint, not a flaw. My work spans analytics, machine learning, and data engineering hence building pipelines, models, and reports that extract structure from noise. I value rigor over hype, explainability over spectacle. Comfortable with ambiguity, I optimize, measure, and iterate until patterns submit to discipline. I am motivated by difficult problems, accountable decisions, and turning controlled chaos into dependable insight for real-world impact contexts.
    </p>
</div>
